...
use frontend::frontend::{Event, Frontend, Side};
...
impl<'a, DM: DepthModel> EdgeDetector<'a, DM> {
    #[allow(dead_code)]
    pub fn new(
        frontend: &'a Frontend<DM>,
        scan_size: u32,
        copy_nums: &[u32],
        alignment: Option<(&str, Option<&str>, u32, f64)>,
    ) -> Self {
        let raw_dep: Box<Vec<i32>> =
            Box::new(frontend.get_scanner().get_raw_window().iter(1).collect());
        let lmq_dep: Box<Vec<i32>> =
            Box::new(frontend.get_scanner().get_low_mq_window().iter(1).collect());
    ...
    fn scan_edge(&self, event: &Event<'a, DM>) -> Vec<(u32, i32)> {
        let (left, mut right) = (
            if event.pos < self.scan_size {
                0
            } else {
                event.pos - self.scan_size
            },
            event.pos + self.scan_size,
        );
    ...
    fn pvalue_validation(&mut self, variant: &Variant) -> Result<f64, ()> {
        if let Some(ref mut bamfile) = self.bamfile {
            ...
            if length > variant.left_pos {
                length = variant.left_pos;
            }

            let range = (
                (variant.left_pos - length) as usize,
                (variant.right_pos + length) as usize,
            );
            ...
            for read in iter {
                ...
                if read.get_flags() & 0x80d != 1 {
                    continue;
                }
                if read.begin() < range.0 as u32 || read.ref_begin() < range.0 as u32 {
                    continue;
                }
                if read.get_isize() > 0 {
                    let beg = read.begin() - range.0 as u32;
                    let end =
                        read.begin() + self.read_size + (read.get_isize() as u32) - range.0 as u32;

                    if end > (range.1 - range.0) as u32 {
                        continue;
                    }
                ...
                if read.ref_end() - range.0 as u32 > (range.1 - range.0) as u32 {
                    continue;
                }

                window_r.accumulate(
                    (read.ref_begin() - range.0 as u32) as usize,
                    (read.ref_end() - range.0 as u32) as usize,
                    1,
                );
            }
            ...
            if exclude_size > 0 {
                let exclude_range = [
                    event_depth_value[exclude_size],
                    event_depth_value[event_depth_value.len() - exclude_size],
                ];

                let score = exclude_range.iter().map(|x| {
                    (match normal_depth_value[..].binary_search_by(|y| fcmp(y, x)) {
                        Err(idx) => idx,
                        Ok(idx) => idx,
                    }) as f64
                        / (normal_depth_value.len() as f64)
                });
    ...
    #[allow(dead_code)]
    pub fn detect_edge<'b>(
        &'b mut self,
        event: &(Event<'a, DM>, Event<'a, DM>),
        retry: bool,
    ) -> Option<Variant<'a>> {
        let left_edges = self.scan_edge(&event.0);
        let right_edges = self.scan_edge(&event.1);

        let raw_dep_sum = self.raw_dep[(event.0.pos as usize)..(event.1.pos as usize)]
            .iter()
            .fold(0.0, |s, v| s + (*v as f64));
        let lmq_dep_sum = self.lmq_dep[(event.0.pos as usize)..(event.1.pos as usize)]
            .iter()
            .fold(0.0, |s, v| s + (*v as f64));

        if lmq_dep_sum > raw_dep_sum * 0.2 && raw_dep_sum > 100000.0 {
            return None;
        }
        ...
        'outer: for (left_pos, left_rate) in (&left_edges[0..]).iter().take(5) {
            for (right_pos, right_rate) in (&right_edges[0..]).iter().take(5) {
                ...
                if (left_rate * 2 < *right_rate || right_rate * 2 < *left_rate)
                    && (*left_rate > 10 || *right_rate > 10)
                {
                    continue;
                }
                ...
                if ret.is_none()
                    || (avg - 0.5 * (copy_num as f64)).abs()
                        < (ret.as_ref().unwrap().mean - 0.5 * (copy_num as f64)).abs()
                    || (avg - lmq_avg - 0.5 * (copy_num as f64)).abs()
                        < (ret.as_ref().unwrap().mean - 0.5 * (copy_num as f64)).abs()
                        ...
                        && (sd < ret.as_ref().unwrap().sd))
                {
                    ...
                    if (avg - 0.5 * (copy_num as f64)).abs()
                        < (best_avg - 0.5 * (copy_num as f64)).abs()
                    {
                        best_avg = avg;
                    }
                    ret = Some(Variant {
        ...
        if ret.is_some() && (0.5 * (copy_num as f64) - ret.as_ref().unwrap().mean).abs() > 0.2 {
            let mut left = event.0.pos;
            let mut right = event.1.pos;

            while (self.histogram.normalize(self.raw_dep[left as usize] as u32)
                - (copy_num as f64) * 0.5)
                .abs()
                > 0.1
                && left < right
            ...
            while (self
                .histogram
                .normalize(self.raw_dep[right as usize] as u32)
                - (copy_num as f64) * 0.5)
                .abs()
                > 0.1
                && left < right
        ...
        if result.iter().fold(1.0, |_x, y| y.pv_score) < self.pv_threshold {
            result = None
        }
        ...
        if retry && result.is_some() && result.as_ref().unwrap().pv_score < 0.0 {
            ...
            for copy_num in self.target_copy_num.clone().iter() {
                if *copy_num == cur_copy_num || *copy_num == result.as_ref().unwrap().copy_num {
                    continue;
                }
