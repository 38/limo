...
fn main() -> Result<(), ()> {
    ...
    let window_size =
        u32::from_str_radix(matches.value_of("window-size").unwrap_or("300"), 10).unwrap();
    ...
    let mut edge_detect = EdgeDetector::new(
        &frontend_ctx.frontend,
        frontend_ctx.frontend.get_scan_size() * 2,
        &copy_nums[0..],
        prob_args,
    );
    ...
    let mut events: Vec<_> = event_pair
        .iter()
        .filter_map(|ep| {
            if ep.0.pos > last_mb {
                eprintln!(
                    "Postprocessed: Chromosome:{}, Offset:{}MB/{}MB, FE_Events:{}, Passed:{}",
                    ep.0.chrom,
                    last_mb / 1000000,
                    total_mb,
                    event_count,
            ...
            event_count += 1;
            edge_detect.detect_edge(ep, true).map(|x| {
                passed += 1;
                x
            })
        })
    ...
    let events = {
        ...
        loop {
            ...
            if let Some(sv) = next_sv {
                if max(sv.left_pos - max_dist, cluster_range.0)
                    < min(sv.right_pos + max_dist, cluster_range.1)
                {
            ...
            if should_merge {
                if cluster.len() > 1 {
                    fn update<'a, 'b>(
                        ...
                        } else if ((best.mean - 0.5 * best.copy_num as f64).abs()
                            - (sv.mean - 0.5 * sv.copy_num as f64).abs())
                        .abs()
                            > 0.05
                        {
                            if (best.mean - 0.5 * best.copy_num as f64).abs()
                                > (sv.mean - 0.5 * sv.copy_num as f64).abs()
                            {
                                return sv;
                            }
                    ...
                    let best = cluster
                        .iter()
                        .skip(1)
                        .fold(cluster[0], |a, b| update(a, *b));

                    /* Option 2: Merge all the SV in the cluster */
                    let event_pair = make_linear_event(
                        cluster[0].chrom,
                        cluster[0].left_pos,
                        cluster[cluster.len() - 1].right_pos,
                        best.copy_num,
                    );
